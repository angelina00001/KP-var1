...
class TestPushService:
    ...
    async def test_send_multiple_push(self):
        ...
        service = MockPushService()
        nonce1 = await service.send_2fa_challenge("token1", 1, "test1@example.com", "Device1")
        nonce2 = await service.send_2fa_challenge("token2", 2, "test2@example.com", "Device2")
        assert nonce1 != nonce2  # Each should be unique

    def test_create_response_payload(self):
        """Test creating response payload"""
        payload = PushService.create_2fa_response_payload(approved=True, nonce="test_nonce")
    ...
    def test_create_response_payload_declined(self):
        ...
        payload = PushService.create_2fa_response_payload(approved=False, nonce="test_nonce")

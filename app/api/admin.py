...
_BTN_STYLE = (
    ...
    "padding: 5px 10px; border-radius: 10px; cursor: pointer; margin: 2px;"
)
_ADMIN_BTN_STYLE = (
    "background: #d4619b; color: white; border: none; "
    "padding: 5px 10px; border-radius: 10px; cursor: pointer; margin: 2px;"
)
...
_ADMIN_STYLES = """
            ...
            .btn {
                display: inline-block; background: #d4619b; color: white;
                padding: 10px 20px; text-decoration: none; border-radius: 25px; margin-top: 15px;
            }
...
_ADMIN_SCRIPT = """
            ...
            async function deleteUser(userId) {
                let warn = 'ВНИМАНИЕ! Вы уверены, что хотите УДАЛИТЬ этого пользователя? ';
...
def _user_actions_html(user: User, current_user: User) -> str:
    if user.tfa_enabled:
        disable_btn = (
            f'<button onclick="disable2FA({user.id})" style="{_BTN_STYLE}">'
            "🔐 Выкл 2FA</button>"
        )
    else:
        disable_btn = '<span style="color: #999;">2FA выкл</span>'

    admin_label = "👑 Снять админа" if user.is_admin else "👤 Сделать админом"
    toggle_btn = (
        f'<button onclick="toggleAdmin({user.id}, {str(user.is_admin).lower()})" '
        f'style="{_ADMIN_BTN_STYLE}">{admin_label}</button>'
    )

    if user.id != current_user.id:
        delete_btn = (
            f'<button onclick="deleteUser({user.id})" style="{_BTN_STYLE}">'
            "🗑️ Удалить</button>"
        )
    else:
        delete_btn = '<span style="color: #999;">(Вы)</span>'

    return f"{disable_btn}\n                {toggle_btn}\n                {delete_btn}"
...
async def admin_dashboard(
    ...
    tfa_users = (
        await db.execute(
            select(func.count()).select_from(User).where(User.tfa_enabled.is_(True))
        )
    ).scalar() or 0
    total_devices = (
    ...
    for u in users:
        ...
        actions = _user_actions_html(u, current_user)
        tfa_icon = "✅" if u.tfa_enabled else "❌"
        ...
        <tr>
            ...
            <td>{u.email}</td>
            <td>{u.full_name}</td>
            <td style="text-align: center;">{tfa_icon}</td>
            <td style="text-align: center;">{role_icon}</td>
